...
class NativeAudioPluginHandler(
    ...
    companion object {
        ...
        private const val FLUTTER_METHOD_ON_LOADED_ARG_STARTED_AUTOMATICALLY =
            "startedAutomatically"
        private const val FLUTTER_METHOD_ON_LOADED_ARG_TOTAL_DURATION_IN_MILLIS =
        ...
        withService { service ->
            when (call.method) {
                NATIVE_METHOD_PLAY -> {
                    withArgument(call, NATIVE_METHOD_PLAY_ARG_URL) { url: String ->
                        ...
                        val startAutomatically =
                            call.argument<Boolean>(NATIVE_METHOD_PLAY_START_AUTOMATICALLY)
                        val startFromMillis =
                            call.argument<Int>(NATIVE_METHOD_PLAY_START_FROM_MILLIS)
                ...
                NATIVE_METHOD_SET_SKIP_TIME -> {
                    withArgument(
                        call,
                        NATIVE_METHOD_SET_SKIP_TIME_ARG_FORWARD_MILLIS
                    ) { forward: Long ->
                        withArgument(
                            call,
                            NATIVE_METHOD_SET_SKIP_TIME_ARG_BACKWARD_MILLIS
                        ) { rewind: Long ->
    ...
    private fun withService(withService: (AudioService) -> Unit) {
        if (audioService == null) {
            ...
            if (!context.isServiceRunning(AudioService::class.java)) context.startService(
                serviceIntent
            )
            context.bindService(
                serviceIntent,
            ...
            onLoaded = { totalDurationInMillis, startedAutomatically ->
                try {
                    channel.invokeMethod(
                        FLUTTER_METHOD_ON_LOADED, mapOf(
